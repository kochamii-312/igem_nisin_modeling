...
def fit_timekill(
    ...
    contour_out: str = "./mnt/data/pHとnisin濃度の影響度/contour_v2_1.png",
    ...
    if make_plots:
        plt.figure()
        plt.scatter(y, yhat)
        plt.xlabel("Observed E. coli log-reduction")
        plt.ylabel("Fitted E. coli log-reduction")
        plt.title("Observed vs. Fitted (Weibull v2)")
        # y=x 対角線を追加
        xy_min = min(np.min(y), np.min(yhat))
        xy_max = max(np.max(y), np.max(yhat))
        plt.plot([xy_min, xy_max], [xy_min, xy_max], color="gray", linestyle="--", linewidth=1)
        plt.savefig("./mnt/data/モデルの性能/fit_hill_obsvsfit.png", dpi=160, bbox_inches="tight")
        plt.close()
    ...
    with open("./mnt/data/parameters_hill.txt", "w", encoding="utf-8") as f:
